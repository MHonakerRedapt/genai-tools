...
class WordDocParser(BaseParser):
    def __init__(self) -> None:
        super().__init__()
    ...
    def parse(self, content: str, file_name: Optional[str] = None) -> Document:
        ...
        return Document(content=cleanup_content(content), title=title)
...
def extract_pdf_content_pypdf(file_path):
...
def extract_worddoc_content(file_path):
    out = subprocess.Popen([os.path.abspath('worddoc_extractor.sh'), os.path.abspath(file_path)], stdout=subprocess.PIPE)
...
def chunk_file(
    ...
    cracked_pdf = False
    if file_format == "pdf":
        if not isinstance(form_recognizer_client,DocumentAnalysisClient):
            print('Using pypdf to extract content from pdf files')
            content = extract_pdf_content_pypdf(file_path)
            cracked_pdf = True
            # raise UnsupportedFormatError("form_recognizer_client is required for pdf files")
        else:
            print('Using Azure Document Intelligence service to extract content from pdf files')
            content = extract_pdf_content(file_path, form_recognizer_client, use_layout=use_layout)
            cracked_pdf = True
    elif file_format == 'worddoc':
        print('Attempting to extract content from Word document')
        content = extract_worddoc_content(file_path)
    else:
        try:
    ...
    def __new__(cls, *args, **kwargs):
        if not cls.instance:
            ...
            if url and key:
                ...
                print("SingletonFormRecognizerClient: Skipping since credentials not provided. Assuming alternative parsing method for PDF files, if any are in directory")
